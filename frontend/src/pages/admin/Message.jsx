...
export default function Message() {
  ...
  // 3. Close parent-list when clicking outside parent-dropdown-wrapper
  useEffect(() => {
    const handleClickOutside = (e) => {
      // Keep dropdown open if click inside dropdown or recipient-selection
      const isInsideDropdown = e.target.closest(".parent-dropdown-wrapper");
      const isInsideRecipientSelection = e.target.closest(
        ".recipient-selection"
      );
      ...
      if (
        showDropdownList &&
        !isInsideDropdown &&
        !isInsideRecipientSelection
      ) {
        setShowDropdownList(false);
      }
    };

    // Use mousedown event which is less likely to be stopped by stopPropagation()
  ...
  const loadMetaData = async () => {
    try {
      ...
      const mappedDrivers = driversData.map((driver, index) => {
        // Lấy routeId từ API
        let realRouteId = parseInt(
          driver.route_id || driver.current_route_id || 0,
          10
        );
        ...
        return {
          ...
          routeName:
            driver.routeName ||
            driver.tuyen_duong ||
            `Tuyến (Gán tạm) ${realRouteId}`,
        };
      });

      // LOG KIỂM TRA: Bạn mở F12 xem dòng này, nếu thấy số [1, 2, 3...] là thành công
      console.log(
        "🔥 Tài xế sau khi gán tuyến:",
        mappedDrivers.map((d) => ({ Ten: d.fullname, Tuyen: d.routeId }))
      );
  ...
  const getFilteredList = () => {
    ...
    if (recipientType === "parent") {
      ...
      if (recipientFilter !== "all") {
        ...
        console.log(
          "📊 Danh sách Route ID của tất cả học sinh trong RAM:",
          checkData
        );
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      await Promise.all(
        selectedMessages.map((id) => NotificationService.deleteMessage(id))
      );
      setMessages((prev) =>
      ...
      alert("Có lỗi khi xóa danh sách tin nhắn");
    }
  };
  ...
  const handleSendMessage = async (isScheduled = false) => {
    ...
    // TRƯỜNG HỢP 2: Chọn thủ công (Tài xế All hoặc Phụ huynh)
      ...
      if (finalRecipients.length === 0 && currentList.length > 0) {
        ...
        if (
          window.confirm(
            `Bạn chưa chọn người cụ thể. Bạn có muốn gửi cho toàn bộ ${currentList.length} người trong danh sách không?`
          )
        ) {
    ...
    <div className="message-page">
      ...
      {showComposeModal && (
        <div
          ...
          onClick={(e) => {
            // Chỉ đóng modal nếu click trên overlay, không phải modal content
                ...
                >
                  ...
                  <div className="recipient-dropdown-area">
                    ...
                    {recipientType === "driver" && recipientFilter !== "all" ? (
                      ...
                      >
                        <i className="fa fa-check-circle"></i> Hệ thống tự động
                        chọn tài xế tuyến:{" "}
                        <strong>
                          {
                            availableRoutes.find((r) => r.id == recipientFilter)
                              ?.name
                          }
                        </strong>
                      </div>
                      ...
                      >
                        ...
                        >
                          <label
                            className="select-all-checkbox-label"
                            style={{ cursor: "pointer" }}
                          >
                            <input
                              type="checkbox"
                              style={{ accentColor: "#007bff", marginRight: 8 }}
                              onChange={handleSelectAll}
                              checked={
                                currentList.length > 0 &&
                                selectedRecipients.length >= currentList.length
                              }
                            />
              ...
              <div className="schedule-summary">
                ...
                <p>
                  ...
                  {recipientType === "driver"
                    ? recipientFilter !== "all"
                      ? ` Tài xế tuyến ${getSelectedRouteName()}`
                      : " Tất cả tài xế"
